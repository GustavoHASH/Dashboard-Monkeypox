...
CENTER_LAT, CENTER_LON = -23.533143292075447, -46.6879224826753

#data load
df_country = pd.read_csv("spMonkey.CSV", sep=";")
df_states = pd.read_csv("spMonkey.CSV", sep=";")

token = open(".mapbox_token").read()

df_states_ = df_states[df_states["data"] == "2022-06-08"]
...
fig = px.choropleth_mapbox(df_states_, locations="estado",
    geojson=brazil_states, center={"lat": -16.95, "lon": -47.78},  # https://www.google.com/maps/ -> right click -> copiar lat/lon
...
app.layout = dbc.Container(
    children=[
        dbc.Row([
            dbc.Col([
                    html.Div([
                        html.H5(children="Evolução do Monkeypox no Estado de São Paulo"),
                ...
                [
                    dcc.Loading(
                        ...
                        children=[dcc.Graph(id="choropleth-map", figure=fig, 
                            style={'height': '150vh', 'margin-right': '10px'})],
                    ),
                ], md=7),
            ], class_name='g-0')
    ], fluid=True, 
...
def display_status(date, location):
    # print(location, date)
    if location == "BRASIL":
        df_data_on_date = df_country[df_country["data"] == date]
    else:
...
def plot_line_graph(plot_type, location):
    if location == "BRA":
        df_data_on_location = df_country.copy()
    else:
